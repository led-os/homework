package yc.com.soundmark.base.presenter;
...
import yc.com.soundmark.study.utils.EngineUtils;
...
public class BasePayPresenter extends BasePresenter<BaseEngine, BasePayContract.View> implements BasePayContract.Presenter {
    ...
    @Override
    public void createOrder(final String payway_name, final String money, final String goods_id, final String title, List<GoodInfo> list) {
        mView.showLoadingDialog("正在创建订单，请稍候...");
        Subscription subscription = EngineUtils.createOrder(mContext, 1, title, money, goods_id).subscribe(new Subscriber<ResultInfo<OrderInfo>>() {
            @Override
