...
/**
 * Created by wanglin  on 2018/11/28 14:09.
 */
public class CameraManger {
    ...
    public static final int ALLOW_PIC_LEN = 1500;       //最大允许的照片尺寸的长度   宽或者高
     ...
     * @param degree
     */
    public void openCamera(SurfaceHolder holder, int degree) {
        try {
            //初始化摄像头
...
//            camera = null;
     ...
     * 设置参数
     */
    public void setCameraParameters(int screenWidth, int screenHeight, Camera.AutoFocusCallback autoFocusCallback) {
        try {
            if (camera != null) {
    ...
    public void turnCamera(SurfaceHolder holder, Camera.AutoFocusCallback autoFocusCallback, int degree, int screenWidth, int screenHeight) {
        ...
        if (camera != null && cameraPosition == Camera.CameraInfo.CAMERA_FACING_BACK) {
            ...
            try {
                ...
                camera.setDisplayOrientation(degree);
//                camera.autoFocus(autoFocusCallback);
                setCameraParameters(screenWidth, screenHeight, autoFocusCallback);
            } catch (IOException e) {
                e.printStackTrace();
            }
                        ...
                        try {
                            ...
                            if (volume != 0) {
                                if (mShootSound == null) {
                                    ...
                                    mShootSound.setAudioStreamType(AudioManager.STREAM_ALARM);
                                    mShootSound.setVolume(0, 0);
                                }
                                if (mShootSound != null) {
                                    mShootSound.start();
                                }
...
//            Toast.makeText(, "Camera被占用，请先关闭", Toast.LENGTH_SHORT).show();
        }
    }
