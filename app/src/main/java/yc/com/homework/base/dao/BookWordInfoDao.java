package yc.com.homework.base.dao;

import android.database.Cursor;
import android.database.sqlite.SQLiteStatement;

import org.greenrobot.greendao.AbstractDao;
import org.greenrobot.greendao.Property;
import org.greenrobot.greendao.internal.DaoConfig;
import org.greenrobot.greendao.database.Database;
import org.greenrobot.greendao.database.DatabaseStatement;

import yc.com.homework.word.model.domain.BookWordInfo;

// THIS CODE IS GENERATED BY greenDAO, DO NOT EDIT.
/** 
 * DAO for table "BOOK_WORD_INFO".
*/
public class BookWordInfoDao extends AbstractDao<BookWordInfo, Long> {

    public static final String TABLENAME = "BOOK_WORD_INFO";

    /**
     * Properties of entity BookWordInfo.<br/>
     * Can be used for QueryBuilder and for referencing column names.
     */
    public static class Properties {
        public final static Property Id = new Property(0, Long.class, "id", true, "_id");
        public final static Property BookId = new Property(1, String.class, "bookId", false, "BOOK_ID");
        public final static Property Name = new Property(2, String.class, "name", false, "NAME");
        public final static Property CoverImg = new Property(3, String.class, "coverImg", false, "COVER_IMG");
        public final static Property VersionId = new Property(4, String.class, "versionId", false, "VERSION_ID");
        public final static Property Period = new Property(5, String.class, "period", false, "PERIOD");
        public final static Property PartType = new Property(6, String.class, "partType", false, "PART_TYPE");
        public final static Property Grade = new Property(7, String.class, "grade", false, "GRADE");
        public final static Property Subject = new Property(8, String.class, "subject", false, "SUBJECT");
        public final static Property IsDel = new Property(9, String.class, "isDel", false, "IS_DEL");
        public final static Property Sort = new Property(10, String.class, "sort", false, "SORT");
        public final static Property Press = new Property(11, String.class, "press", false, "PRESS");
        public final static Property SentenceCount = new Property(12, String.class, "sentenceCount", false, "SENTENCE_COUNT");
        public final static Property GradeName = new Property(13, String.class, "gradeName", false, "GRADE_NAME");
        public final static Property VersionName = new Property(14, String.class, "versionName", false, "VERSION_NAME");
        public final static Property SaveTime = new Property(15, long.class, "saveTime", false, "SAVE_TIME");
        public final static Property Type = new Property(16, int.class, "Type", false, "TYPE");
    }


    public BookWordInfoDao(DaoConfig config) {
        super(config);
    }
    
    public BookWordInfoDao(DaoConfig config, DaoSession daoSession) {
        super(config, daoSession);
    }

    /** Creates the underlying database table. */
    public static void createTable(Database db, boolean ifNotExists) {
        String constraint = ifNotExists? "IF NOT EXISTS ": "";
        db.execSQL("CREATE TABLE " + constraint + "\"BOOK_WORD_INFO\" (" + //
                "\"_id\" INTEGER PRIMARY KEY ," + // 0: id
                "\"BOOK_ID\" TEXT," + // 1: bookId
                "\"NAME\" TEXT," + // 2: name
                "\"COVER_IMG\" TEXT," + // 3: coverImg
                "\"VERSION_ID\" TEXT," + // 4: versionId
                "\"PERIOD\" TEXT," + // 5: period
                "\"PART_TYPE\" TEXT," + // 6: partType
                "\"GRADE\" TEXT," + // 7: grade
                "\"SUBJECT\" TEXT," + // 8: subject
                "\"IS_DEL\" TEXT," + // 9: isDel
                "\"SORT\" TEXT," + // 10: sort
                "\"PRESS\" TEXT," + // 11: press
                "\"SENTENCE_COUNT\" TEXT," + // 12: sentenceCount
                "\"GRADE_NAME\" TEXT," + // 13: gradeName
                "\"VERSION_NAME\" TEXT," + // 14: versionName
                "\"SAVE_TIME\" INTEGER NOT NULL ," + // 15: saveTime
                "\"TYPE\" INTEGER NOT NULL );"); // 16: Type
    }

    /** Drops the underlying database table. */
    public static void dropTable(Database db, boolean ifExists) {
        String sql = "DROP TABLE " + (ifExists ? "IF EXISTS " : "") + "\"BOOK_WORD_INFO\"";
        db.execSQL(sql);
    }

    @Override
    protected final void bindValues(DatabaseStatement stmt, BookWordInfo entity) {
        stmt.clearBindings();
 
        Long id = entity.getId();
        if (id != null) {
            stmt.bindLong(1, id);
        }
 
        String bookId = entity.getBookId();
        if (bookId != null) {
            stmt.bindString(2, bookId);
        }
 
        String name = entity.getName();
        if (name != null) {
            stmt.bindString(3, name);
        }
 
        String coverImg = entity.getCoverImg();
        if (coverImg != null) {
            stmt.bindString(4, coverImg);
        }
 
        String versionId = entity.getVersionId();
        if (versionId != null) {
            stmt.bindString(5, versionId);
        }
 
        String period = entity.getPeriod();
        if (period != null) {
            stmt.bindString(6, period);
        }
 
        String partType = entity.getPartType();
        if (partType != null) {
            stmt.bindString(7, partType);
        }
 
        String grade = entity.getGrade();
        if (grade != null) {
            stmt.bindString(8, grade);
        }
 
        String subject = entity.getSubject();
        if (subject != null) {
            stmt.bindString(9, subject);
        }
 
        String isDel = entity.getIsDel();
        if (isDel != null) {
            stmt.bindString(10, isDel);
        }
 
        String sort = entity.getSort();
        if (sort != null) {
            stmt.bindString(11, sort);
        }
 
        String press = entity.getPress();
        if (press != null) {
            stmt.bindString(12, press);
        }
 
        String sentenceCount = entity.getSentenceCount();
        if (sentenceCount != null) {
            stmt.bindString(13, sentenceCount);
        }
 
        String gradeName = entity.getGradeName();
        if (gradeName != null) {
            stmt.bindString(14, gradeName);
        }
 
        String versionName = entity.getVersionName();
        if (versionName != null) {
            stmt.bindString(15, versionName);
        }
        stmt.bindLong(16, entity.getSaveTime());
        stmt.bindLong(17, entity.getType());
    }

    @Override
    protected final void bindValues(SQLiteStatement stmt, BookWordInfo entity) {
        stmt.clearBindings();
 
        Long id = entity.getId();
        if (id != null) {
            stmt.bindLong(1, id);
        }
 
        String bookId = entity.getBookId();
        if (bookId != null) {
            stmt.bindString(2, bookId);
        }
 
        String name = entity.getName();
        if (name != null) {
            stmt.bindString(3, name);
        }
 
        String coverImg = entity.getCoverImg();
        if (coverImg != null) {
            stmt.bindString(4, coverImg);
        }
 
        String versionId = entity.getVersionId();
        if (versionId != null) {
            stmt.bindString(5, versionId);
        }
 
        String period = entity.getPeriod();
        if (period != null) {
            stmt.bindString(6, period);
        }
 
        String partType = entity.getPartType();
        if (partType != null) {
            stmt.bindString(7, partType);
        }
 
        String grade = entity.getGrade();
        if (grade != null) {
            stmt.bindString(8, grade);
        }
 
        String subject = entity.getSubject();
        if (subject != null) {
            stmt.bindString(9, subject);
        }
 
        String isDel = entity.getIsDel();
        if (isDel != null) {
            stmt.bindString(10, isDel);
        }
 
        String sort = entity.getSort();
        if (sort != null) {
            stmt.bindString(11, sort);
        }
 
        String press = entity.getPress();
        if (press != null) {
            stmt.bindString(12, press);
        }
 
        String sentenceCount = entity.getSentenceCount();
        if (sentenceCount != null) {
            stmt.bindString(13, sentenceCount);
        }
 
        String gradeName = entity.getGradeName();
        if (gradeName != null) {
            stmt.bindString(14, gradeName);
        }
 
        String versionName = entity.getVersionName();
        if (versionName != null) {
            stmt.bindString(15, versionName);
        }
        stmt.bindLong(16, entity.getSaveTime());
        stmt.bindLong(17, entity.getType());
    }

    @Override
    public Long readKey(Cursor cursor, int offset) {
        return cursor.isNull(offset + 0) ? null : cursor.getLong(offset + 0);
    }    

    @Override
    public BookWordInfo readEntity(Cursor cursor, int offset) {
        BookWordInfo entity = new BookWordInfo( //
            cursor.isNull(offset + 0) ? null : cursor.getLong(offset + 0), // id
            cursor.isNull(offset + 1) ? null : cursor.getString(offset + 1), // bookId
            cursor.isNull(offset + 2) ? null : cursor.getString(offset + 2), // name
            cursor.isNull(offset + 3) ? null : cursor.getString(offset + 3), // coverImg
            cursor.isNull(offset + 4) ? null : cursor.getString(offset + 4), // versionId
            cursor.isNull(offset + 5) ? null : cursor.getString(offset + 5), // period
            cursor.isNull(offset + 6) ? null : cursor.getString(offset + 6), // partType
            cursor.isNull(offset + 7) ? null : cursor.getString(offset + 7), // grade
            cursor.isNull(offset + 8) ? null : cursor.getString(offset + 8), // subject
            cursor.isNull(offset + 9) ? null : cursor.getString(offset + 9), // isDel
            cursor.isNull(offset + 10) ? null : cursor.getString(offset + 10), // sort
            cursor.isNull(offset + 11) ? null : cursor.getString(offset + 11), // press
            cursor.isNull(offset + 12) ? null : cursor.getString(offset + 12), // sentenceCount
            cursor.isNull(offset + 13) ? null : cursor.getString(offset + 13), // gradeName
            cursor.isNull(offset + 14) ? null : cursor.getString(offset + 14), // versionName
            cursor.getLong(offset + 15), // saveTime
            cursor.getInt(offset + 16) // Type
        );
        return entity;
    }
     
    @Override
    public void readEntity(Cursor cursor, BookWordInfo entity, int offset) {
        entity.setId(cursor.isNull(offset + 0) ? null : cursor.getLong(offset + 0));
        entity.setBookId(cursor.isNull(offset + 1) ? null : cursor.getString(offset + 1));
        entity.setName(cursor.isNull(offset + 2) ? null : cursor.getString(offset + 2));
        entity.setCoverImg(cursor.isNull(offset + 3) ? null : cursor.getString(offset + 3));
        entity.setVersionId(cursor.isNull(offset + 4) ? null : cursor.getString(offset + 4));
        entity.setPeriod(cursor.isNull(offset + 5) ? null : cursor.getString(offset + 5));
        entity.setPartType(cursor.isNull(offset + 6) ? null : cursor.getString(offset + 6));
        entity.setGrade(cursor.isNull(offset + 7) ? null : cursor.getString(offset + 7));
        entity.setSubject(cursor.isNull(offset + 8) ? null : cursor.getString(offset + 8));
        entity.setIsDel(cursor.isNull(offset + 9) ? null : cursor.getString(offset + 9));
        entity.setSort(cursor.isNull(offset + 10) ? null : cursor.getString(offset + 10));
        entity.setPress(cursor.isNull(offset + 11) ? null : cursor.getString(offset + 11));
        entity.setSentenceCount(cursor.isNull(offset + 12) ? null : cursor.getString(offset + 12));
        entity.setGradeName(cursor.isNull(offset + 13) ? null : cursor.getString(offset + 13));
        entity.setVersionName(cursor.isNull(offset + 14) ? null : cursor.getString(offset + 14));
        entity.setSaveTime(cursor.getLong(offset + 15));
        entity.setType(cursor.getInt(offset + 16));
     }
    
    @Override
    protected final Long updateKeyAfterInsert(BookWordInfo entity, long rowId) {
        entity.setId(rowId);
        return rowId;
    }
    
    @Override
    public Long getKey(BookWordInfo entity) {
        if(entity != null) {
            return entity.getId();
        } else {
            return null;
        }
    }

    @Override
    public boolean hasKey(BookWordInfo entity) {
        return entity.getId() != null;
    }

    @Override
    protected final boolean isEntityUpdateable() {
        return true;
    }
    
}
